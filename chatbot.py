...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='models/clinic',
                        help='model directory to store checkpointed models')
    parser.add_argument('-n', type=int, default=500,
                       help='number of characters to sample')
    parser.add_argument('--prime', type=str, default="Doctor: Hello, what seems to be the problem?\nPatient:",
                        help='prime text')
...
def sample_main(args):
    ...
    # 1) 读取并兼容 config：dict -> argparse.Namespace
    with open(config_path, 'rb') as f:
        saved_args = pickle.load(f)
    if isinstance(saved_args, dict):
        # 用 Namespace 包一层，方便 Model 使用点号属性
        import argparse as _argparse
        saved_args = _argparse.Namespace(**saved_args)

    # 2) 读取并兼容 vocab：支持 dict 或 (chars, vocab)
    with open(vocab_path, 'rb') as f:
        loaded = pickle.load(f)
    if isinstance(loaded, dict):
        # train.py 保存格式
        chars, vocab = loaded['chars'], loaded['vocab']
    else:
        # 旧格式：直接是 (chars, vocab) 元组
        chars, vocab = loaded

    # 3) 建模（推理模式下 Model 会把 seq_length/batch_size 置为 1）
    print("Creating model...")
    # 注意：beam search 的并行数由生成器控制，不需要改 saved_args.batch_size
    ...
    with tf.Session(config=config) as sess:
        ...
        # 可选：打印一下关键超参以确认一致性
        # print(f"[INFO] blocks={saved_args.num_blocks}, block_size={saved_args.block_size}, "
        #       f"layers={saved_args.num_layers}, vocab_size={saved_args.vocab_size}")

        if args.prime:
            print("\n" + args.prime, end="", flush=True)
